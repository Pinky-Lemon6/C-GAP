...
- "Causally depends" means:
  1. The target Action is executing the intent of the source Instruction (Direct Execution).
  2. The target uses data generated by the source (Data Flow).
  3. The target is a logical downstream step in a plan defined by the source.
...
**Crucial for Instruction-Action Pairs:**
- If the Source is an INSTRUCTION (e.g., User/Orchestrator) and the Target is an ACTION (e.g., WebSurfer), ask yourself: **"Is this action an attempt to fulfill that instruction?"**
- If YES, output depends=true, even if they are far apart in time.
...
def _step_type(step: StandardLogItem) -> str:
    """Infer step type using FUZZY matching for complex role strings."""
    
    # 1. Normalize
    role = (step.role or "").strip().lower()
    
    # 2. Check Instruction Roles (Keyword Search)
    # 只要 role 字符串里包含 "orchestrator" 就认，不管后面有没有括号
    for keyword in INSTRUCTION_ROLES:
        if keyword in role:
            return "instruction"

    # 3. Check Action Roles (Keyword Search)
    for keyword in ACTION_ROLES:
        if keyword in role:
            return "action"

    # 4. Fuzzy fallback
    if "agent" in role or "surfer" in role:
        return "action"

    # 5. Fallback to fields
    iaot = step.parsed_iaot
    instruction = getattr(iaot, "instruction", None)
    action = getattr(iaot, "action", None)
    
    # 注意：这里加个防抖，防止空的 instruction 被误判
    if instruction and len(str(instruction)) > 5 and not action:
        return "instruction"
    if action:
        return "action"
